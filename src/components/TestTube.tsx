...
import { Chemical } from '../context/SimulationContext'; // استيراد الواجهة

interface TestTubeProps {
  substances: { substance: Chemical; volume: number }[];
  height?: number;
  width?: number;
  className?: string;
  showBubbles?: boolean;
  showSmoke?: boolean;
  showIce?: boolean; // إضافة الخاصية
  shake?: boolean;
  glow?: boolean;
  glowColor?: string;
  showFire?: boolean;
...
export const TestTube: React.FC<TestTubeProps> = ({
  ...
  showBubbles = false,
  showSmoke = false,
  showIce = false, // تعيين قيمة افتراضية
  shake = false,
  glow = false,
  glowColor,
  showFire = false,
  ...
  pouringTubeX = -width * 0.5,
  pouringTubeY = -height * 0.8,
}) => {
  useEffect(() => {
  ...
  const [pouringSubstance, setPouringSubstance] = useState<Chemical | null>(null);
        ...
        {showIce && (
          <div className="absolute inset-0 z-20 pointer-events-none">
            {Array.from({ length: 10 }).map((_, i) => (
              <motion.div
                key={i}
                className="absolute rounded-lg bg-white/40"
                style={{
                  width: Math.random() * 20 + 10,
                  height: Math.random() * 15 + 10,
                  left: `${Math.random() * 100}%`,
